...
FROM summary
WHERE summary.nps < 0
  AND summary.total_reviews > 10
  AND summary.review_month = '2022-02-01'
;
...
-- NPS
--
-- Top 20 summary FOR CURRENT MONTH, anything that scored 0 TO -100
--
-- NEED TO RUN MONTH QUERY FIRST THEN THE OTHERS

SET month_filter = '2022-03-01';
--
WITH summary AS (
    SELECT COALESCE(ssa.company_name, t.concept_name)                                     AS company_concept_name,
           DATE_TRUNC('month', ubr.review_date)                                           AS review_month,
...
-- Review level - customer information (PII)
...
SELECT DATE_TRUNC('month', ubr.review_date) AS review_month,
       ...
       COUNT(*)
FROM se.data.user_booking_review ubr
    LEFT JOIN se.data.fact_booking fb ON ubr.booking_id = fb.booking_id
GROUP BY 1, 2
;
