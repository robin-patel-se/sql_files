...
DROP TABLE IF EXISTS donald_gatfield.members CASCADE
;
CREATE UNLOGGED TABLE donald_gatfield.members
(
...
COPY donald_gatfield.members_1 (id, member_id, sign_up_date, last_updated, original_territory, current_territory, schedule_tstamp, extracted_at)
...
(
    id              SERIAL NOT NULL
        CONSTRAINT bookings_pkey PRIMARY KEY,
    member_id       varchar(32),
...
CREATE INDEX idx_events_cookie_id_event_tstamp ON donald_gatfield.events (cookie_id, event_tstamp)
;
CREATE INDEX idx_events_member_id ON donald_gatfield.events (member_id)
;
...
DROP TABLE IF EXISTS donald_gatfield.session_identifier CASCADE;
CREATE UNLOGGED TABLE donald_gatfield.session_identifier
AS
WITH event_lags AS (
    SELECT member_id,
           cookie_id,
           ...
           page_urlpath
    FROM donald_gatfield.events
    ORDER BY cookie_id ASC,
             event_tstamp ASC
)
SELECT member_id,
       cookie_id,
...
CREATE INDEX idx_session_identifier_master_session_identifier_covering ON donald_gatfield.session_identifier (master_session_identifier, event_tstamp, page_urlpath)
;

CREATE INDEX idx_session_identifier_session_identifier_member_id ON donald_gatfield.session_identifier (member_id)
;


-- RP: Event level table
...
DROP VIEW IF EXISTS donald_gatfield.top_ten_session_landing_pages;
CREATE VIEW donald_gatfield.top_ten_session_landing_pages
AS
SELECT session_identifier.page_urlpath        AS session_first_landing_page,
...
FROM (
         ...
         SELECT master_session_identifier,
                MIN(event_tstamp) AS first_event
         FROM donald_gatfield.session_identifier
         GROUP BY master_session_identifier
     ) landing_event_in_session
         JOIN donald_gatfield.session_identifier
...
SELECT *
FROM donald_gatfield.top_ten_session_landing_pages
;
...
--RP: session level table
DROP VIEW IF EXISTS donald_gatfield.session_events_first_last;
CREATE VIEW donald_gatfield.session_events_first_last
AS
SELECT session_members.member_id,
...
--RP: session count, differs from session count in q2
SELECT COUNT(*)
FROM donald_gatfield.session_events_first_last sefl;
-- sessions 224721

-- encapsulate the question logic in a view (i.e. if this were part of a wider system)
DROP VIEW IF EXISTS donald_gatfield.session_conversion_rate;
CREATE VIEW donald_gatfield.session_conversion_rate
AS
SELECT SUM(1)   AS number_of_distinct_sessions,
       SUM(CASE
...
FROM donald_gatfield.session_events_first_last AS sessions
         LEFT JOIN donald_gatfield.bookings AS complete_bookings
...
SELECT *
FROM donald_gatfield.session_conversion_rate
;
...
-- encapsulate the question logic in a view (i.e. if this were part of a wider system)
DROP VIEW IF EXISTS donald_gatfield.member_sessions_event_age_buckets;
...
FROM donald_gatfield.session_identifier AS sessions
	...
	FROM members
	GROUP BY member_id
) distinct_members ON members.extracted_at = distinct_members.extracted_at
        AND members.member_id = sessions.member_id
    INNER JOIN members ON members.member_id = distinct_members.member_id
INNER JOIN (
...
SELECT *
FROM donald_gatfield.member_sessions_event_age_buckets
;
